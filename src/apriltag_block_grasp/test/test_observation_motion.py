...
def config():
    return ObservationMotionConfig(
        pose_deg={"b": 0.0, "s": 0.0, "e": 70.0, "t": 90.0},
        move_order=("b", "t", "s", "e"),
        speed_deg_s=35.0,
        acceleration=35.0,
...
def test_exact_hardware_validated_sequence_is_planned_without_gripper():
    ...
    assert [item["joint"] for item in commands] == [1, 4, 2, 3]
    assert [item["angle"] for item in commands] == [0.0, 90.0, 0.0, 70.0]
    assert all(item["T"] == 121 for item in commands)
    assert all(item["spd"] == 35.0 and item["acc"] == 35.0 for item in commands)
    assert all(item["joint"] not in (5, 6) for item in commands)
...
def test_r_and_gripper_are_not_valid_observation_pose_joints():
    with pytest.raises(ValueError, match="only b, s, e and t"):
        ObservationMotionConfig(
            pose_deg={
                "b": 0.0,
                "s": 0.0,
