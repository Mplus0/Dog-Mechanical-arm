...
dog-arm/
├── README.md                         # 本说明文档
├── source_red_block.sh               # 现场环境 source 脚本，路径写死为 /home/sunrise/dog/ros2_red_block_ws
├── docs/
│   └── ros1_ros2_bridge_arm_side.md  # ROS1/ROS2 bridge 只桥接比赛高层话题的说明
├── config_runtime/
...
这是当前唯一标准 ROS2 功能包，类型是 `ament_python`。它负责相机读取、红色目标检测、目标三维定位、RoArm-M3 串口控制、视觉伺服抓取任务和比赛高层任务接口。
...
| `launch/competition_arm_task.launch.py` | 启动比赛任务接口和视觉伺服，读取 `competition_arm.yaml` |
| `launch/competition_arm_ros2.launch.py` | 比赛 ROS2 侧完整启动，包含 `/dog_arm/task_cmd` 高层命令接口 |
...
| `competition_arm_task.launch.py` | `roarm_driver_node`、`target_localizer_node`、`visual_servo_task_node`、`competition_arm_task_node`、日志节点 | 比赛任务接口测试 |
| `competition_arm_ros2.launch.py` | 与比赛相关的完整 ROS2 侧启动，`enable_pick_place_sequence` 默认为 true | ROS2 侧接收 `/dog_arm/task_cmd` 的比赛流程 |
...
- `ros1_bridge`：仅在需要 ROS1 机器狗端与 ROS2 机械臂端通信时需要；
...
以下命令假设仓库是 ROS2 工作空间根目录，且功能包位于 `src/red_block_grasp_ros2`：
...
```bash
ros2 launch red_block_grasp_ros2 localization_only.launch.py start_roarm_driver:=false show_window:=true
```
...
根据当前代码，机械臂与机器狗的通信采用 **高层任务 topic + JSON 字符串协议**。机械臂内部的相机、目标定位、串口控制、视觉伺服等 topic 不直接暴露给机器狗。
...
机器狗只需要关心下面 3 个 topic，类型均为 `std_msgs/String`，消息内容是 JSON 字符串。
...
- `src/red_block_grasp_ros2/red_block_grasp_ros2/nodes/competition_arm_task_node.py`
- `src/red_block_grasp_ros2/config/competition_arm.yaml`
- `src/red_block_grasp_ros2/docs/dog_arm_bridge.yaml`
- `docs/ros1_ros2_bridge_arm_side.md`
...
### 8.5 ROS1/ROS2 bridge 策略

当前仓库预留的 bridge 配置只桥接 3 个高层 topic：

```yaml
topics:
  -
    topic: /dog_arm/task_cmd
    type: std_msgs/msg/String
    queue_size: 10
  -
    topic: /dog_arm/task_result
    type: std_msgs/msg/String
    queue_size: 10
  -
    topic: /dog_arm/base_adjust_req
    type: std_msgs/msg/String
    queue_size: 10
```

启动建议：在机械臂侧运行 ROS2 任务和 `ros1_bridge`，机器狗侧保持 ROS1。
...
export ROS_MASTER_URI=http://<机器狗ROS1_IP>:11311
export ROS_IP=<机械臂侧IP>
...
这样做的好处是机器狗侧不需要理解机械臂内部状态机，也不会误发底层机械臂命令；双方只通过任务、结果和底盘微调请求解耦。
...
| 更严格的通信协议 | 后续可从 `std_msgs/String` JSON 升级为自定义 msg/srv/action，但当前 bridge 最简单稳定的是 String |
...
ros2 launch red_block_grasp_ros2 visual_servo_task.launch.py \
  model_path:=<your_ws>/src/red_block_grasp_ros2/models/red_block_yolo11n_v3_mix.pt \
  handeye_path:=<your_ws>/src/red_block_grasp_ros2/handeye/handeye_cam_to_eef.json
```
...
chmod +x src/red_block_grasp_ros2/red_block_grasp_ros2/tools/calibrate_red_threshold.py
```
...
1. `src/red_block_grasp_ros2/setup.py`：理解 ROS2 Python 节点如何注册为 `ros2 run` 命令。
2. `src/red_block_grasp_ros2/launch/visual_servo_task.launch.py`：理解一个完整任务如何同时启动多个节点。
3. `src/red_block_grasp_ros2/red_block_grasp_ros2/nodes/roarm_driver_node.py`：理解机械臂命令 topic 的 JSON 协议。
4. `src/red_block_grasp_ros2/red_block_grasp_ros2/nodes/target_localizer_node.py`：理解相机、检测、深度定位、手眼转换。
5. `src/red_block_grasp_ros2/red_block_grasp_ros2/nodes/visual_servo_task_node.py`：理解抓取状态机。
...
| ROS1/ROS2 bridge 话题 | `docs/ros1_ros2_bridge_arm_side.md`、`src/red_block_grasp_ros2/docs/dog_arm_bridge.yaml` |
| 官方 MoveIt2/RoArm 桥接 | `dog_visual_grasp_bridge/visual_moveit_grasp.py`、`dog_visual_grasp_bridge/config/grasp_config.yaml` |
| 新增 ROS2 节点 | 在 `red_block_grasp_ros2/nodes/` 新增文件，并在 `setup.py` 的 `console_scripts` 注册 |

如果要新增 launch 文件，放在 `src/red_block_grasp_ros2/launch/` 下即可，`setup.py` 已经通过 `glob("launch/*.launch.py")` 安装所有 launch 文件。
