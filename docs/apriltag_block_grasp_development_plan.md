...
- 正常的新一轮 `pick` 会再次进入固定观察姿态；观察动作只发送 B/T/S/E，R 和 G 保持不动。驱动返回定时完成后才开始定位，重定位等待中的重复或覆盖命令不重复该动作。
...
- 阶段 5.2 首次空载开爪候选参数沿用相同硬件旧包：`110°`、`25°/s`、加速度 `25`、定时等待 `1.0 s`。该参数只允许通过独立语义命令实测，验收前不接入自动 `pick`，闭爪仍禁用。
- 阶段 5.2 独立空载开爪已通过后，任务节点可在稳定目标快照冻结后执行一次开爪；驱动确认完成后进入预抓取规划，但不得继续移动到预抓取点。
- 阶段 5.3 先进行只读规划：以冻结物块坐标加上非对称夹爪 TCP 偏移得到最终夹持点，再仅将 Z 增加候选预抓取高度。旧包中的 `120 mm` 仅作为有来源的候选值；输出 `pre_grasp_plan_ready` 时不得发送任何笛卡尔运动命令。
- 阶段 5.3 只读规划通过并获得用户批准后，仅开放分段 `T=104` 空载运动到预抓取点。沿用旧包 `0.18` 速度、120 mm 单段硬上限和 25 mm XYZ 到位容差；规划步长预留到位容差，因此不超过 95 mm。每个预抓取平移段从发送前的新鲜 `T=1051` 状态复制当前 `tit/r/g`，不在该段同时切换到最终抓取姿态；最终抓取姿态留到后续接近阶段单独验证。到达预抓取点后必须终止本轮，不得继续接近、下降、闭爪或抬升。
- 阶段 5.4 的接近点由现场确认设置为最终夹持点沿 base Z 正方向 `80 mm`，并与预抓取点、最终夹持点保持相同 XY。首个小阶段只把 `approach_tcp_mm` 加入规划结果，并明确输出 `approach_motion_enabled=false`；未单独批准前不得从预抓取点移动到接近点。
- 为缩短后续开发周期，接近、最终夹持、闭爪和相对抬升代码一次实现，通过有序 `execution_limit`（`pre_grasp`、`approach`、`final_grasp`、`close_gripper`、`lift`）逐级放行。驱动与任务节点必须配置相同限位，任一侧越级均拒绝命令。默认限位为 `approach`。接近、最终夹持和抬升的 T=104 命令使用标定的固定 pitch/roll；该协议不含 yaw/B 字段，B 仍由固件逆解选择。完整 `lift` 结束后才发布 `pick_success`、写入 `picked_ids` 并设置 `carrying_id`；节点重启清空记录。

### 10.4 放置与机器狗通信（后续补充）

本轮不实现放置运动和最终狗端协议。后续开始前必须重新确认：

- 放置使用 ABCD 贴纸识别还是四个固定坐标；
- `place` 命令、结果和重试 JSON 格式，以及与 ROS Noetic 机器狗的兼容桥接；
- 放置点、释放高度、开爪参数、撤离高度和到位规则；
- `placed_ids`、`carrying_id` 清理，以及两个 ID 均完成后的周期自动清空时点。

当前收到非 `pick` 命令时，任务节点只返回
`place_and_dog_communication_not_implemented`，不得发送机械臂运动。
...
现场观察位姿的补充 100 帧静态数据为 X/Y/Z 峰峰值
`[0.7714, 0.8750, 1.6080] mm`，标准差 `[0.1615, 0.1855, 0.3536] mm`。
因此稳定阈值更新为 `[1.0, 1.0, 2.0] mm`：仅放宽有实测依据的 Z 轴，
X/Y 继续保持 1 mm。
